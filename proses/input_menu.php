  ...
  if ($statusUpload == 0) {
    ...
    if (mysqli_num_rows($select) > 0) {
      ...
      if (move_uploaded_file($_FILES['foto']['tmp_name'], $target_file)) {
        $query = mysqli_query($db, "INSERT INTO tb_menu (foto,nama_menu,keterangan,kategori,harga) values ('" . $kode_random . $_FILES['foto']['name'] . "','$nama_menu','$keterangan','$kategori','$harga')");
        if ($query) {
